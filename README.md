...
Use the following command in the API Debugger tool, and generate an API user 
Use the text below (use your own name) within the API Debugger and press the POST button.
...
``` 
Now press the button on the bridge and then press the POST button again and you should get a succesful response like below.
...
The command we are using will add an entry to the login keychain and set "Always allow access by these applications:" list within the Access Control preferences.
...
For more info/options please see the man page for **security**.


We have all the info we need to fill in the global variables within the script
...
After starting a meeting in zoom, we got extra UDP connections.
After doing many tests of starting and ended many meetings, UDP connections are only there when a meeting is active. 
...
## But what if my camera and mic are off?

When my camera and mic are off the UDP connections where still there. But we can monitor if the camera or mic is off and then create that case to be "not in a meeting"

## Create an Automator app that loops this script.

Now we have a script that scan's for running meetings, calls, all we need to do is loop this.
